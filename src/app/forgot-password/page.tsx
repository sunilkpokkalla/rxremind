      ...
      <div className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-20 xl:px-24 bg-white z-10 shadow-xl">
        <div className="mx-auto w-full max-w-md">
          ...
          >
            ...
            {state?.success && (
              <div className="p-4 bg-emerald-50 border border-emerald-500/10 text-emerald-800 text-sm font-medium rounded-xl space-y-1">
                <p className="font-bold">Check your inbox!</p>
                <p className="text-slate-600">{state.message}</p>
              </div>
            )}

            {/* Simulated Inbox Box for Walkthrough Demo */}
            {state?.success && state?.simulatedLink && (
              <div className="p-4 bg-primary-light/50 border border-primary/10 rounded-2xl shadow-sm animate-fade-in">
                <div className="flex items-start space-x-3">
                  <Sparkles className="h-5 w-5 text-primary mt-0.5 animate-pulse-soft" />
                  <div>
                    <h4 className="text-sm font-bold text-primary-dark">Simulated Reset Email (Demo Mode)</h4>
                    <p className="text-xs text-slate-600 mt-1 leading-normal">
                      Since you are currently in mock fallback database mode, password recovery is fully simulated. Click below to continue directly to password reset!
                    </p>
                    <Link
                      href={state.simulatedLink}
                      className="mt-3 inline-flex items-center px-4 py-2 text-xs font-bold text-white bg-primary hover:bg-primary-hover active:bg-primary-dark rounded-xl transition duration-150 shadow-sm shadow-primary/10"
                    >
                      Reset Password Now
                    </Link>
                  </div>
